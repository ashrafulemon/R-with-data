...
bb= read.xlsx("I:\\1_upwork\\004_work with Jinu_____data analysis\\demo_R_Copy\\bb.xlsx",
              sheet=1,colNames=T, detectDates=F) 

bb=bb[,c(1,9)]

names(bb)[1] <- "Item.#"
names(bb)[2] <- "Unit.of.Issue"

jn_gv_aj$BPA.Category <- ifelse(
  jn_gv_aj$`Item.#` %in% bb$`Item.#`,
  "BPA",
  "Non-BPA"
)

bb <- bb[!duplicated(bb[1],fromLast= T), ] 
...
##################################################
ju_aj111= jn_gv_aj[c(6,9)]
bb1<- left_join(ju_aj111, bb, by = "Item.#")
bb11=bb1[3]
bb00=bb1[2]

bb11[bb11 == ""] <- NA
bb11[is.na(bb11)] <- bb00[is.na(bb11)]

jn_gv_aj=cbind(jn_gv_aj,bb11)
jn_gv_aj=jn_gv_aj[c(1:8,18,10:17)]
names(jn_gv_aj)[9] <- "Unit.of.Issue"

#################################################


#3 set for combine
jn_aj_gv_out = jn_gv_aj[c(3:17)]   #33##################3
r_aj_3v_out = aj123_r_f[3:17] #28
r_aj_5v_out = aj12345_r_f[3:17]  #28
